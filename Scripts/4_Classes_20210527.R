...
# Approach: 1) Analyze class abundance. Lipids are grouped by class
#           2) Fold change and stats 
#           3) Plots TG


# Goal: Generate table with all lipid class changes and plot TG abundance changes
...
plot<- filter(ptclass_table, grepl("TG",Var)) #change to class

Classes<-ggplot(plot,aes(x = Group, y = Val, fill = Group)) +
  geom_boxplot(outlier.alpha = 0.99, outlier.size=1.5)+
  xlab("")+
  ylab("Triglyceride abundance (ng/mg_prot)")+  #change y-axis labelling
  theme_bw()+
...
ggsave("../Output_Figures/4_Class_TG.pdf", Classes, width=0.8, height=2, units="in", scale=3) #change filename
