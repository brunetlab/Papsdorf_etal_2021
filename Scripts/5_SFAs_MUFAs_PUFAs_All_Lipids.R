######## SFAs MUFAs PUFAs in all lipids #################################################

# Data:2_Lipids_Imputed_CVfiltered (Output_Data)
#      Lipids are normalized, imputed, CV_filtered

# Approach: 1) Fatty acid abundance in all lipids (individual fatty acid concentration matches the corresponding lipid). 
...
#           5) Plot SFAs, MUFAs and PUFAs. Code is repetetive


# Goal: Plot SFAs, MUFAs and PUFAs


# Output:  Pdf: 5_SFAs_MUFAs_PUFAs_All_Lipids.pdf
#          Csv: 5_SFAs_MUFAs_PUFAs_All_Lipids.csv   
...
#Save
write.csv(SMP_table, '../Output_Data/5_SFAs_MUFAs_PUFAs_All_Lipids.csv')
...
######## 5) Plots SFAs, MUFAs and PUFAs         #################################################


######## Subset and plot SFAs  

SFA<- filter(ptSMP_table, grepl("SFA",Var)) 

SFA<-ggplot(SFA,aes(x = Group, y = Val, fill = Group)) +
  geom_boxplot(outlier.alpha = 0.99, outlier.size=1.5)+
  xlab("")+
  ylab("SFA abundance all lipids (ng/mg_prot)")+  
  ylim(55000,150000)+ #to have always the same axis
  theme_bw()+
...
######## Subset and plot MUFAs         #################################################

MUFA<- filter(ptSMP_table, grepl("MUFA",Var))
MUFA<- filter(MUFA, !grepl("MUFAtoPUFA",Var)) 

MUFA<-ggplot(MUFA,aes(x = Group, y = Val, fill = Group)) +
  geom_boxplot(outlier.alpha = 0.99, outlier.size=1.5)+
  xlab("")+
  ylab("MUFA abundance all lipids(ng/mg_prot)")+  
  ylim(55000,150000)+ #to have always the same axis
  theme_bw()+
  theme(axis.text.x = element_text(angle = 50, vjust = 0.5), plot.title = element_text(hjust = 0.5))+
  theme(text = element_text(size=15))+
...
######## Subset and plot PUFAs         #################################################

PUFA<- filter(ptSMP_table, grepl("PUFA",Var)) 
PUFA<- filter(PUFA, !grepl("MUFAtoPUFA",Var)) 

PUFA<-ggplot(PUFA,aes(x = Group, y = Val, fill = Group)) +
  geom_boxplot(outlier.alpha = 0.99, outlier.size=1.5)+
  xlab("")+
  ylab("PUFA abundance all lipids (ng/mg_prot)")+  
  ylim(55000,150000)+ #to have always the same axis
  theme_bw()+
  theme(axis.text.x = element_text(angle = 50, vjust = 0.5), plot.title = element_text(hjust = 0.5))+
  theme(text = element_text(size=15))+
...
ggsave("../Output_Figures/5_SFAs_MUFAs_PUFAs_All_Lipids.pdf", SMP, width=2.5, height=2, units="in", scale=3) 
...
#   [1] tidyselect_1.1.1  haven_2.4.1       carData_3.0-4     colorspace_2.0-1  vctrs_0.3.8       generics_0.1.0    utf8_1.2.1        rlang_0.4.11      pillar_1.6.1      foreign_0.8-71    glue_1.4.2        withr_2.4.2       DBI_1.1.1         dbplyr_2.1.1     
# [15] modelr_0.1.8      readxl_1.3.1      lifecycle_1.0.0   munsell_0.5.0     ggsignif_0.6.1    gtable_0.3.0      cellranger_1.1.0  zip_2.1.1         rvest_1.0.0       labeling_0.4.2    rio_0.5.26        curl_4.3.1        fansi_0.4.2       broom_0.7.6      
# [29] Rcpp_1.0.6        backports_1.2.1   scales_1.1.1      jsonlite_1.7.2    abind_1.4-5       farver_2.1.0      fs_1.5.0          digest_0.6.27     hms_1.1.0         stringi_1.6.2     openxlsx_4.2.3    rstatix_0.7.0     cowplot_1.1.1     grid_3.6.1       
# [43] cli_2.5.0         tools_3.6.1       magrittr_2.0.1    crayon_1.4.1      car_3.0-10        pkgconfig_2.0.3   ellipsis_0.3.2    data.table_1.13.6 xml2_1.3.2        reprex_2.0.0      lubridate_1.7.10  assertthat_0.2.1  httr_1.4.2        rstudioapi_0.13  
# [57] R6_2.5.0          compiler_3.6.1 
