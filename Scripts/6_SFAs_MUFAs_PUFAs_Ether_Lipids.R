######## SFAs MUFAs PUFAs in Ether lipids #################################################

# Data:2_Lipids_Imputed_CVfiltered (Output_Data)
#      Lipids are normalized, imputed, CV_filtered

# Approach: 1) Fatty acid abundance in ether lipids (individual fatty acid concentration matches the corresponding lipid).
...
#           5) Plot SFAs, MUFAs and PUFAs. This code is repetetive.
#           6) Plot MUFA to PUFA ratio


# Goal: Plot Plot SFAs_MUFAs_PUFAs and MUFAstoPUFAs


# Output:  Pdf: 6_SFAs_MUFAs_PUFAs_Ether_Lipids.pdf
#               6_MUFAtoPUFA_Ether_lipids
#          Csv: 6_SFAs_MUFAs_PUFAs_Ether_Lipids.csv   
...
#Save
write.csv(SMP_table, '../Output_Data/6_SFAs_MUFAs_PUFAs_Ether_Lipids.csv')
...
######## 5) Plot SFAs, MUFAs, PUFAs         #################################################


######## Plot SFAs                        

SFAs<- filter(ptSMP_table, grepl("SFA",Var)) 

SFAs<-ggplot(SFAs,aes(x = Group, y = Val, fill = Group)) +
  geom_boxplot(outlier.alpha = 0.99, outlier.size=1.5)+
  xlab("")+
  ylab("SFA abundance ether lipids (ng/mg_prot)")+   
  ylim(1000,25000)+ #to have always the same axis
  theme_bw()+
...
MUFAs<- filter(ptSMP_table, grepl("MUFA",Var)) 
MUFAs<- filter(MUFAs, !grepl("MUFAtoPUFA",Var)) 

MUFAs<-ggplot(MUFAs,aes(x = Group, y = Val, fill = Group)) +
  geom_boxplot(outlier.alpha = 0.99, outlier.size=1.5)+
  xlab("")+
  ylab("MUFA abundance ether lipids (ng/mg_prot)")+  
  ylim(1000,25000)+ #to have always the same axis
  theme_bw()+
  theme(axis.text.x = element_text(angle = 50, vjust = 0.5), plot.title = element_text(hjust = 0.5))+
  theme(text = element_text(size=15))+
...
PUFAs<- filter(ptSMP_table, grepl("PUFA",Var)) 
PUFAs<- filter(PUFAs, !grepl("MUFAtoPUFA",Var)) 

PUFAs<-ggplot(PUFAs,aes(x = Group, y = Val, fill = Group)) +
  geom_boxplot(outlier.alpha = 0.99, outlier.size=1.5)+
  xlab("")+
  ylab("PUFA abundance ether lipids(ng/mg_prot)")+ 
  ylim(1000,25000)+ #to have always the same axis
  theme_bw()+
  theme(axis.text.x = element_text(angle = 50, vjust = 0.5), plot.title = element_text(hjust = 0.5))+
  theme(text = element_text(size=15))+
...
######## Plot SFAs MUFAs PUFAs together  

SMP<-ggarrange(SFAs, MUFAs, PUFAs, 
                    ncol = 3, nrow = 1)
...
ggsave("../Output_Figures/6_SFAs_MUFAs_PUFAs_Ether_Lipids.pdf", SMP, width=2.4, height=2, units="in", scale=3) 
...
MUFAtoPUFA<- filter(ptSMP_table, grepl("MUFAtoPUFA",Var)) 

MUFAtoPUFA<-ggplot(MUFAtoPUFA,aes(x = Group, y = Val, fill = Group)) +
  geom_boxplot(outlier.alpha = 0.99, outlier.size=1.5)+
  xlab("")+
  ylab("MUFAstoPUFAs ratio ether lipids")+ 
  theme_bw()+
  theme(axis.text.x = element_text(angle = 50, vjust = 0.5), plot.title = element_text(hjust = 0.5))+
  theme(text = element_text(size=15))+
  theme(legend.position = "none")+
...
ggsave("../Output_Figures/6_MUFAtoPUFA_Ether_Lipids.pdf", MUFAtoPUFA, width=0.8, height=2, units="in", scale=3) 
...
#   [1] tidyselect_1.1.0 haven_2.3.1      carData_3.0-4    colorspace_1.4-1 vctrs_0.3.7      generics_0.1.0   utf8_1.1.4      
# [8] rlang_0.4.10     pillar_1.6.0     foreign_0.8-76   glue_1.4.2       withr_2.4.2      DBI_1.1.0        dbplyr_2.1.1    
# [15] modelr_0.1.8     readxl_1.3.1     lifecycle_1.0.0  munsell_0.5.0    ggsignif_0.6.1   gtable_0.3.0     cellranger_1.1.0
# [22] zip_2.1.1        rvest_1.0.0      rio_0.5.26       labeling_0.4.2   curl_4.3         fansi_0.4.1      broom_0.7.6     
# [29] Rcpp_1.0.5       backports_1.2.0  scales_1.1.1     jsonlite_1.7.2   abind_1.4-5      farver_2.0.3     fs_1.5.0        
# [36] hms_1.0.0        digest_0.6.27    openxlsx_4.2.3   stringi_1.5.3    rstatix_0.7.0    cowplot_1.1.1    grid_3.6.3      
# [43] cli_2.5.0        tools_3.6.3      magrittr_2.0.1   car_3.0-10       crayon_1.4.1     pkgconfig_2.0.3  ellipsis_0.3.2  
# [50] MASS_7.3-53      xml2_1.3.2       reprex_2.0.0     lubridate_1.7.10 assertthat_0.2.1 httr_1.4.2       rstudioapi_0.13 
